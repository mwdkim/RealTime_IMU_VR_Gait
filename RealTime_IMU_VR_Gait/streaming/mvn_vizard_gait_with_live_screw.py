...
axis.visible(viz.ON)
# Live screw-axis line
# ---------- Screw axis geometry ----------
screw_axis_line = viz.startLayer(viz.LINES)
viz.vertex(0,0,0)
viz.vertex(0,0,0)
screw_axis_line = viz.endLayer()

screw_axis_line.color(viz.RED)
screw_axis_line.visible(viz.ON)
...
def screw_udp_loop():
    global latest_r0, latest_e, latest_h

    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    s.bind(("0.0.0.0", SCREW_UDP_PORT))
    viz.logNotice(f"Listening for live screw axis on UDP {SCREW_UDP_PORT}")
...
threading.Thread(target=screw_udp_loop, daemon=True).start()
def update_screw_axis():

    if latest_r0 is None or latest_e is None:
        return
    ...
    p1 = [px - L*ex, py - L*ey, pz - L*ez]
    p2 = [px + L*ex, py + L*ey, pz + L*ez]

    screw_axis_line.setVertex(0,p1)
    screw_axis_line.setVertex(1,p2)

    # color by pitch magnitude
    ah = abs(latest_h)

    if ah < 0.01:
        screw_axis_line.color(viz.GREEN)
    elif ah < 0.03:
        screw_axis_line.color(viz.YELLOW)
    else:
        screw_axis_line.color(viz.RED)
