...
PITCH_UDP_IP = "127.0.0.1"
PITCH_UDP_PORT = 5055
pitch_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

SCREW_UDP_IP = "127.0.0.1"
SCREW_UDP_PORT = 5056
screw_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
try:
    while total < MAX_STEPS:
        ...
        msg = f"{r0[0]:.6f},{r0[1]:.6f},{r0[2]:.6f},{e[0]:.6f},{e[1]:.6f},{e[2]:.6f},{h:.6f}"
        screw_sock.sendto(msg.encode("utf-8"), (SCREW_UDP_IP, SCREW_UDP_PORT))

        pitch_msg = f"{h:.6f}".encode("utf-8")
        pitch_sock.sendto(pitch_msg, (PITCH_UDP_IP, PITCH_UDP_PORT))
